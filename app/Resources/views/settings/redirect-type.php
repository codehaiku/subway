...
<div id="subway_redirect_page_option_section" class="hidden subway-redirect-option-section">
    ...
    <p class="description">
		...
		<?php _e( 'The selected page will be used as the redirect endpoint for all of your users. 
		Selecting blank (-) will redirect the user to the default redirect defined in WordPress or other plugins.
		 Choose "Custom URL" if you want to redirect to a custom URL or domain.', 'subway' ); ?>
...
<div id="subway_redirect_custom_url_option_section" class="hidden subway-redirect-option-section">
    ...
    <input value="<?php echo esc_attr( esc_url( get_option( 'subway_redirect_custom_url' ) ) ); ?>" type="text"
           name="subway_redirect_custom_url" placeholder="<?php esc_attr_e( 'http://', 'subway' ); ?>"
           id="subway_redirect_custom_url" size="75"/>

    <p class="description"><br>

		<?php
		echo sprintf( __( 'When entering a custom domain, you can use a variable string such us: %1$s and %2$s.
For example, http://yoursiteurl.com/members/<strong>%2$s</strong> will
 translate to http://yoursiteurl/members/<strong>admin</strong> where "admin" is equal to the %2$s 
 variable; http://yoursiteurl.com/users/<strong>%1$s</strong> will translate to 
 http://yoursiteurl.com/users/<strong>4</strong> where "4" is equal to the %1$s. 
 Both variables refer to the current user that is logged-in.', 'subway' ), '%user_id%', '%user_name%' );
		?>
    ...
    <p class="description">
		...
		<?php esc_attr_e( 'Leave empty to use existing WordPress or other 3rd party
		 plugin redirect option.', 'subway' ); ?>

    </p><br>

    <p class="description">

		<?php esc_html_e( 'Warning: External URLs are not supported by WordPress Function (wp_safe_redirect) 
		and will be redirected back to default WordPress behavior.', 'subway' ); ?>
